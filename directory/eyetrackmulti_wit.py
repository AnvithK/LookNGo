...
def listen_in_background():
    with sr.Microphone() as source:
        ...
        while True:  # Infinite loop to continuously listen
            try:
                audio = r.listen(source,phrase_time_limit=1)  # Listen for the first phrase and extract it into audio data
                text = r.recognize_wit(audio)  # Use Google Web Speech API to recognize the speech
                print(text)
