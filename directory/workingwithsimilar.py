    ...
    with sr.Microphone() as source:
        ...
        while app_running:
            try:
                ...
                commands = ["click", "right click", "start", "stop", "scroll on", "scroll off"]

                # Find the closest match to the recognized text within the list of commands
                closest_match = difflib.get_close_matches(text, commands, n=1, cutoff=0.6)  # cutoff can be adjusted for accuracy

                if closest_match:
                    command = closest_match[0]  # Take the closest match as the command
